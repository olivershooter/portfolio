...
- **Audience**: engineering hiring managers, technical leads, recruiters at tech-forward companies
...
- **`gap-px bg-outline` bento grid trick**: Instead of adding borders to each cell individually (which causes double-borders and misaligned widths), the grid container uses a 1px gap with the outline colour as background — the gap _is_ the border.
